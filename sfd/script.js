...
var onLoadFunction = function(){
  ...
  var drawDiagram = function(beamType) {
    ...
    var drawMomentDiagram = function(dots) {
      ...
      var minMoment = Math.min(...dots.map(dot => dot.m));
      ...
      for (var i = 0; i < dots.length - 1; i++) {
        ...
        var y1 = cY + 660 - (dot1.m - minMoment) / maxMoment * diagMaxValue;
        var y2 = cY + 660 - (dot2.m - minMoment) / maxMoment * diagMaxValue;
    
        drawLine(dot1.x, y1, dot2.x, y2, 1, "MidnightBlue");
      }
    
      dots.forEach(function(dot, index) { // Draw dots and grid lines (optional)
        var y = cY + 660 - (dot.m - minMoment) / maxMoment * diagMaxValue;
        drawDot(dot.x, y);
        if (index % 20 == 0) {
